...
| **Claude Desktop** | CC Switch DB（`claude-desktop` 行，**直连模式**） | 由 Claude Desktop 端选择 | CC Switch 接管时自动写 `Claude-3p` 网关配置；Claude Desktop 需切到第三方（3p）部署模式 |
...
- **Claude Desktop + GOAT 套餐**：打开 Claude Desktop 可能提示「Couldn't sign in to 网关 / Gateway rejected the configured credential (HTTP 403)」——这不是 Key 错误，而是 Claude Desktop 用 `claude-haiku-4-5` 等模型探测网关时，上游返回 `MODEL_NOT_IN_PLAN`（GOAT 不含任何 Claude 模型，Pro 及以上才含）。**升级套餐即可用，无需重跑脚本**；配置本身已正确写入。
